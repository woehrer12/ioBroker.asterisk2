...
class TextToGSMConverter {
  ...
  constructor(transcoder) {
    this.options = {};
    this.transcoder = transcoder == 'sox' ? 'sox' : 'ffmpeg';
  }
  ...
  textToMp3Stream(text, language, volume) {
    return new Promise((resolve, reject) => {
      ...
      // https://forum.iobroker.net/topic/45177/asterisk-bringt-error-while-dialing-2-error/115?_=1680090164625&lang=de
      let sounddata = '';
  ...
  mp3ToGsmFfmpeg(fileNameMP3, fileNameGSM, unlink = true) {
    return new Promise((resolve, reject) => {
  ...
  mp3ToGsmSox(fileNameMP3, fileNameGSM, unlink = true) {
    return new Promise((resolve, reject) => {
      const ffmpeg = childProcess.spawn(
        'sox',
        [
  ...
  textToGsm(text, language, volume, fileNameGSM, unlink = true) {
    ...
    return new Promise((resolve, reject) => {
      this.textToMp3(text, language, volume, fileNameMP3)
        .then((result) => {
          if (this.transcoder == 'ffmpeg') {
            this.mp3ToGsmFfmpeg(fileNameMP3, fileNameGSM, unlink)
              .then((result) => resolve(result))
              .catch((err) => reject(err));
          } else if (this.transcoder == 'sox') {
            this.mp3ToGsmSox(fileNameMP3, fileNameGSM, unlink)
              .then((result) => resolve(result))
              .catch((err) => reject(err));
          } else {
            reject('Transcoder missing');
          }
        })
        .catch((err) => reject(err));
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise.all(promisArray).then((values) => {
        ...
        if (reason) {
          ...
          reject(new Error(`Promise Error in textToMp3: ${text}, ${language}, ${volume}, 0`));
        }
      });
